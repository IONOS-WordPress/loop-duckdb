...
-- persist the s3 json files in duckdb table loop_items
...
FROM
  (
    SELECT
      ...
      CASE
        WHEN json_extract_string(json_content, '$.instance') IS NOT NULL THEN json_extract_string(json_content, '$.instance')
        ELSE NULL
      END AS instance,
