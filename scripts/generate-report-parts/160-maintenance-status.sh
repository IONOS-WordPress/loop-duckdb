...
#
# generates security settings related questions markdown output for "Essentials Maintenance Status"
# Pie chart of status
# 
...
WITH 
  maintenance_counts AS (
    ...
    SELECT
      JSON_EXTRACT_STRING(plugin_data, '$.ionos-essentials.maintenance') AS extracted_status,
  ...
  scaffolded_results AS (
    ...
    FROM
      migration_statuses s
    LEFT JOIN
      maintenance_counts c
...
readonly TITLE="Essentials Maintenance Status"
...
$(echo $(ionos.loop-duckdb.exec_duckdb "$SQL" '-json') | jq -r --arg title "$TITLE" '
  ...
  (.[] | "  \"\(.migration_status)\" : \(.unique_instance_count)")
')
\`\`\`
EOF
